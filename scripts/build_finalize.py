...
Reads the bundled UI assets only ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â never modifies them.
...
def finalize_project(project_root):
    ...
    js_path = p(project_root, 'retro', 'interface', 'assets', 'Sortable.min.js')
    html_path = p(project_root, 'retro', 'interface', 'assets', 'index.html')
    main_pyw_path = p(project_root, 'main.pyw')

    helpers_py = p(project_root, 'retro', 'utils', 'helpers.py')
    config_py = p(project_root, 'retro', 'utils', 'config.py')
    logger_py = p(project_root, 'retro', 'utils', 'logger.py')
    main_py = p(project_root, 'retro', 'interface', 'window.py')
    api_py = p(project_root, 'retro', 'interface', 'bridge.py')
    preset_py = p(project_root, 'retro', 'engine', 'presets.py')
    flag_mgr_py = p(project_root, 'retro', 'engine', 'flags.py')
    ...
    if os.path.isfile(main_pyw_path):
        ...
        print('WARNING: main.pyw not found ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â skipping S7 stamp (will remain '
...
def finalize_s5(project_root, api_py):
    """Second pass: re-import window after S2 was stamped, hash its
    __init__.co_code, and stamp that into api.py's _SHARD_S5_A/B."""
    sys.path.insert(0, project_root)
    try:
        for mod in list(sys.modules):
            if mod.startswith('retro.interface.window'):
                del sys.modules[mod]
        from retro.interface import window
        co_hash = compute_bytecode_hash(window.MainWindow.__init__)
    finally:
...
def validate(project_root):
    ...
    try:
        for mod in list(sys.modules):
            if mod.startswith('retro.'):
                del sys.modules[mod]
        from retro.utils import helpers
        from retro.interface import window, bridge
        from retro.engine import presets, flags
        from retro.utils import logger as logger_mod, config

        import unittest.mock as mock
        with mock.patch.object(helpers, '_is_frozen', return_value=True):
            helpers._rot_reset()
            helpers._shard_s1_reset()
            window._shard_s2_reset()
            presets._shard_s4_reset()
            api._shard_s5_reset()
            logger_mod._shard_s6_reset()
            flags._shard_s7_reset()
            helpers.get_resource_path('retro/interface/assets/Sortable.min.js')
            window._shard_s2_check()
            presets._shard_s4_check()
            api._shard_s5_check()
            logger_mod._shard_s6_tick()
            flags._shard_s7_check()
            import tempfile
...
if __name__ == '__main__':
    ...
    finalize_s5(args.project,
                os.path.join(args.project, 'retro', 'interface', 'bridge.py'))
