...
#define MyAppName "Retro Injector"
; Kept in sync with version.json by scripts/update_version.py at release time.
#define MyAppVersion "4.2.0"
#define MyAppPublisher "Retro Injector"
#define MyAppURL ""
#define MyAppExeName "RetroInjector.exe"
...
OutputDir=.
OutputBaseFilename=RetroInjector_Installer
...
AppMutex=RetroInjector_SingleInstance_Mutex
...
; C:\Program Files\Retro Injector\ actually goes away every time.
Type: filesandordirs; Name: "{app}\*"
Type: dirifempty;    Name: "{app}"

[Files]
; This takes the output from PyInstaller (dist/RetroInjector/*)
Source: "dist\RetroInjector\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
...
procedure RemoveRetroInjectorHandler(const Scheme: String);
var
  Cmd: String;
begin
  if RegQueryStringValue(HKCU, 'Software\Classes\' + Scheme + '\shell\open\command', '', Cmd) then
  begin
    if (Pos('--roblox-handler', Cmd) > 0) or (Pos('RetroInjector.exe', Cmd) > 0) then
...
begin
  ...
  begin
    RemoveRetroInjectorHandler('roblox-player');
    RemoveRetroInjectorHandler('roblox');
  end;
