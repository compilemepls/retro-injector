import pytest
from retro.engine.version_changer import bootstrapper as bs
...
def test_classify_handler_Retro Injector():
    assert bs.classify_handler(r'"C:\\Apps\\Retro Injector\\FFlagManager.exe" --roblox-handler "%1"') == "Retro Injector"
...
@pytest.mark.parametrize("handler,fixable,expected", [
    ...
    ("Retro Injector", False, False),       # already ours -> no-op
...
def test_register_covers_both_schemes(monkeypatch):
    ...
    bs.register(r"C:\app\RetroInjector.exe")
    for scheme in bs._SCHEMES:
        assert store[scheme] == r'"C:\app\RetroInjector.exe" --roblox-handler "%1"'
...
def test_register_then_restore_roundtrip(monkeypatch):
    ...
    backup = bs.register(r"C:\Apps\Retro Injector\FFlagManager.exe")
...
def test_restore_deletes_when_no_backup(monkeypatch):
    store = _fake_registry(monkeypatch, {
        "roblox-player": r'"C:\Apps\Retro Injector\FFlagManager.exe" --roblox-handler "%1"',
        "roblox": r'"C:\Apps\Retro Injector\FFlagManager.exe" --roblox-handler "%1"',
    })
    bs.restore(None)
    assert store == {}
...
def test_restore_accepts_legacy_string_backup(monkeypatch):
    ...
    store = _fake_registry(monkeypatch, {
        "roblox-player": r'"C:\app\RetroInjector.exe" --roblox-handler "%1"',
        "roblox": r'"C:\app\RetroInjector.exe" --roblox-handler "%1"',
    })
...
def test_register_frozen_command_has_no_script(monkeypatch):
    ...
    store = _fake_registry(monkeypatch, {s: None for s in bs._SCHEMES})
    bs.register(r"C:\app\RetroInjector.exe")
    assert store["roblox-player"] == r'"C:\app\RetroInjector.exe" --roblox-handler "%1"'
...
def test_register_source_command_includes_script(monkeypatch):
    ...
    assert bs.classify_handler(store["roblox-player"]) == "Retro Injector"
