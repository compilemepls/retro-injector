...
from retro.utils.helpers import is_fps_flag
...
def test_set_framerate_cap_replaces_existing():
    out = fps.set_framerate_cap(SAMPLE, 9999)
...
def test_set_framerate_cap_inserts_when_absent():
    ...
    out = fps.set_framerate_cap(no_cap, 9999)
    assert '<int name="FramerateCap">9999</int>' in out


def test_is_unlocked():
    assert fps.is_unlocked(fps.set_framerate_cap(SAMPLE)) is True
    assert fps.is_unlocked(SAMPLE) is False
