...
from retro.engine import presets
...
class TestShardS4(unittest.TestCase):
    def setUp(self):
        ...
        rel = os.path.join('retro', 'interface', 'assets', 'index.html')
        target = os.path.join(self.tmp, rel)
        os.makedirs(os.path.dirname(target), exist_ok=True)
        shutil.copy(os.path.join(FIXTURES, 'clean_index.html'), target)
        presets._SHARD_S4_EXPECTED = _expected_region_hash(target)
    ...
    def test_missing_script_tag_does_not_subtract(self):
        target = os.path.join(self.tmp, 'retro', 'interface', 'assets', 'index.html')
