...
from retro.interface import bridge as api_module
...
class TestAutoVersionOnceReciprocalRestoreGuard(unittest.TestCase):
    def test_missing_backup_does_not_delete_scheme(self):
        ...
        with mock.patch('retro.interface.bridge.Config.save_settings'), \
             mock.patch(
                 'retro.engine.version_changer.bootstrapper.current_handler_class',
                 return_value='Retro Injector'), \
             mock.patch(
                 'retro.engine.version_changer.bootstrapper.restore') as m_restore:
