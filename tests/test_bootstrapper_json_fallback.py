...
the bootstrapper already wrote there. Retro Injector's flags win conflicts; the
...
def test_bootstrapper_fallback_merges_existing_flags(isolated_dirs, monkeypatch):
    ...
    Retro Injector_flags = {
        "FFlagFromRetro Injector": "Retro Injector-value",
        "FFlagShared": "Retro Injector-wins",   # overrides bootstrapper's
    }
    ok, _ = RobloxManager.apply_fflags_json(Retro Injector_flags)

    assert ok is True
    written = _read_settings(isolated_dirs["boot"])
    assert written["FFlagBootstrapperOwn"] == "bloxstrap-value", \
        "bootstrapper's own flags must survive the merge"
    assert written["FFlagFromRetro Injector"] == "Retro Injector-value", \
        "Retro Injector's new flags must land"
    assert written["FFlagShared"] == "Retro Injector-wins", \
        "Retro Injector must win on key collision so live-memory + JSON stay consistent"


def test_stock_dir_still_wins_when_present(isolated_dirs, monkeypatch):
    """User has stock install AND a bootstrapper install. Retro Injector must write to
    ...
    assert not os.path.isfile(boot_settings), \
        "bootstrapper dir must not receive Retro Injector's writes when stock exists"
...
def test_bootstrapper_merge_tolerates_corrupt_existing_json(isolated_dirs, monkeypatch):
    ...
    NOT crash; overwrite with Retro Injector's payload rather than leaving Roblox with
