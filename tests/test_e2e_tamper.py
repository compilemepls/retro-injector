...
from retro.utils import helpers, logger as logger_mod, config
...
class TestE2ETamperFlow(unittest.TestCase):
    def setUp(self):
        ...
        for rel, fixture in [
            (os.path.join('retro', 'interface', 'assets', 'Sortable.min.js'),
             'clean_polyfill.js'),
            (os.path.join('retro', 'interface', 'assets', 'index.html'),
             'clean_index.html'),
        ]:
        ...
        with open(os.path.join(self.tmp, 'retro', 'interface', 'assets',
        ...
        window._SHARD_S2_EXPECTED = full_hash
        ...
        presets._SHARD_S4_EXPECTED = hashlib.sha256(region).digest()
        api._SHARD_S5_EXPECTED = hashlib.sha256(
            window.MainWindow.__init__.__code__.co_code
        ).digest()
        ...
        with open(os.path.join(self.tmp, 'main.pyw'), 'wb') as _f:
            _f.write(fake_main)
        flags._SHARD_S7_EXPECTED = hashlib.sha256(fake_main).digest()
    ...
    def tearDown(self):
        ...
        window._SHARD_S2_EXPECTED = None
        logger_mod._SHARD_S6_EXPECTED = None
        presets._SHARD_S4_EXPECTED = None
        api._SHARD_S5_EXPECTED = None
        flags._SHARD_S7_EXPECTED = None
    ...
    def _run_all_shards(self):
        ...
        helpers.get_resource_path('retro/interface/assets/Sortable.min.js')  # S1
        window._shard_s2_check()                                     # S2
        # S3 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â settings HMAC check via get_content_filter
    ...
    def test_tamper_then_relaunch_stays_dirty(self):
        ...
        helpers._rot_reset()
        self._reset_shards()
        target = os.path.join(self.tmp, 'retro', 'interface', 'assets',
                              'Sortable.min.js')
