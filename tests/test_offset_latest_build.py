...
def test_fetch_latest_build_returns_embedded_clientversion(monkeypatch):
    ...
    # Act
    result = offsets.fetch_latest_build()
...
def test_fetch_latest_build_uses_header_form(monkeypatch):
    ...
    # Act / Assert
    assert offsets.fetch_latest_build() == "version-deadbeef"
...
def test_fetch_latest_build_none_when_all_sources_fail(monkeypatch):
    ...
    # Act / Assert
    assert offsets.fetch_latest_build() is None
...
def test_fetch_latest_build_skips_body_without_version(monkeypatch):
    ...
    # Act / Assert
    assert offsets.fetch_latest_build() == "version-second"
...
def test_load_known_flag_names_seeds_last_source_build(monkeypatch):
    """The startup path (flags.load_offsets -> load_known_flag_names)
    ...
    # Act
    names = offsets.load_known_flag_names()
    ...
    assert offsets.last_source_build() == "version-startuprecord"
    assert offsets.last_source_id() == "imtheo_requests"
...
def test_load_known_flag_names_does_not_clobber_prior_source_build(monkeypatch):
    ...
    offsets.reset_cache()
    offsets._last_source_build = "version-authoritative"
    offsets._last_source_id = "load_offsets_prior"
    ...
    # Act
    offsets.load_known_flag_names()

    # Assert: the earlier authoritative value is preserved.
    assert offsets.last_source_build() == "version-authoritative"
    assert offsets.last_source_id() == "load_offsets_prior"
...
def test_load_known_flag_names_falls_back_to_disk_cache_build(monkeypatch, tmp_path):
    ...
    assert offsets.last_source_build() == "version-fromcache"