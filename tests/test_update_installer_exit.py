...
installer launch so Setup can replace the running RetroInjector.exe.

ShellExecuteW is mocked; os._exit is mocked so the test process stays alive.
"""
import ctypes

from retro.interface import bridge as api_mod
from retro.interface.bridge import Api
from retro.utils import updater
...
def test_launch_helper_exits_on_shell_success(monkeypatch):
    ...
    result = updater._launch_installer_and_exit(r"C:\Temp\Setup_Retro_4.1.0.exe")
    ...
    assert verb == "open"
    assert path.endswith("Setup_Retro_4.1.0.exe")
...
def test_download_update_exits_on_launch_success(tmp_path, monkeypatch):
    ...
    written = tmp_path / "Setup_Retro_4.1.0.exe"
