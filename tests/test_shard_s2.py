...
# pywebview/pystray imports inside window.py will fail in CI/test
...
class TestShardS2(unittest.TestCase):
    def setUp(self):
        ...
        rel = os.path.join('retro', 'interface', 'assets', 'Sortable.min.js')
        ...
        with open(os.path.join(FIXTURES, 'clean_polyfill.js'), 'rb') as f:
            window._SHARD_S2_EXPECTED = hashlib.sha256(f.read()).digest()
    ...
    def test_honeypot_constant_preserved(self):
        # Reverser-bait ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â must keep its exact value
        self.assertEqual(
            window._AD_SCRIPT_HASH,
