...
``RobloxManager.get_roblox_version_string()`` Ã¢â‚¬â€ the DISK-NEWEST build
...
def test_guard_reads_running_pid_build_not_disk(fm, monkeypatch):
    ...
    MUST fire Ã¢â‚¬â€ the previous, disk-based check would have missed this."""
    ...
    # Disk-latest matches offsets Ã¢â‚¬â€ a disk-based guard would say "aligned".
    ...
    assert stub.memory_calls == 0, (
        ...
        f"memory_calls={stub.memory_calls} Ã¢â‚¬â€ the guard was fooled by the disk "
...
def test_guard_stays_off_when_running_matches_offsets(fm, monkeypatch):
    """Sanity: running-build matches offsets Ã¢â€ â€™ live memory runs normally."""
    from retro.engine.roblox import RobloxManager
    ...
    assert stub.memory_calls >= 1, (
        ...
        "offsets Ã¢â‚¬â€ the guard misfired."
    )
...
def test_guard_treats_unknown_running_as_no_signal(fm, monkeypatch):
    ...
    assert stub.memory_calls >= 1, (
        "Guard must not block on an unknown running-build reading Ã¢â‚¬â€ that "
        "signal is ambiguous and blocking would nuke headless / edge cases."
    )


def test_helper_is_no_op_when_rm_missing(fm):
    """`_live_writes_gated(None)` returns (False, "") Ã¢â‚¬â€ a caller with no RM
...
def test_helper_falls_back_to_self_rm_when_arg_omitted(fm, monkeypatch):
    """Hotkey loop calls `_live_writes_gated()` with no arg Ã¢â‚¬â€ the helper
