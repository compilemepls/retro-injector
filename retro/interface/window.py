...
def _shard_s2_check():
    ...
    if not _s2_helpers._is_frozen():
        return
    path = _s2_helpers.get_resource_path('retro/interface/assets/Sortable.min.js')
    try:
...
class MainWindow:
    def __init__(self):
        from retro.utils.helpers import _rot_bootstrap
        _rot_bootstrap()
        _shard_s2_check()
        self.api = Api()

        # Path to HTML UI using resource resolver
        ui_path = get_resource_path(os.path.join('retro', 'interface', 'assets', 'index.html'))
