...
_VERSION_TAG = b'Retro Injector-cfg-v1'
...
class Config:
    ...
    DEFAULT_SETTINGS = {
        ...
        #   "launch_only"  - Retro Injector launches the matched build directly (default, safe)
        #   "bootstrapper" - Retro Injector handles roblox-player:// web joins (Phase 4)
        "roblox_fix_mode": "launch_only",
        # Automatic launch (B1): when True, Retro Injector registers itself as the
        # roblox-player:// handler so clicking Play on the website launches
        # through Retro Injector, and the background loop keeps that registration healed.
        # Default False = Retro Injector never seizes the Play handler on its own; it only
