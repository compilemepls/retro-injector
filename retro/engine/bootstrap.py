...
def launch_join(uri):
    ...
    Whatever happens, if Retro Injector only holds the launcher transiently (seized from a
    ...
    """
    from retro.engine.roblox import RobloxManager
    from retro.engine.version_changer import fixer, deployment, fastpath
    ...
    try:
        ...
        try:
            ...
            # falling back to JSON-only. See flags.apply_flags_hybrid.
...
def bootstrap_join(uri):
    ...
    This is what makes clicking Play apply flags without opening Retro Injector."""
    launch_join(uri)
