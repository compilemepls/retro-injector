...
from retro.utils.helpers import infer_type, clean_flag_name, is_fps_flag, get_flag_prefix, heal_dflag_flag_names

import hashlib as _hashlib_s7
from retro.utils import helpers as _s7_helpers
...
def _fps_unlock_on():
    ...
    Read once per apply/sync ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â never per-flag in a loop."""
    try:
...
class FlagManager:
    ...
    def _live_writes_gated(self, rm=None):
        ...
        `rm` is the RobloxManager to interrogate ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â passed explicitly by
        ...
        The signal is a build-guid mismatch between the offsets Retro Injector has
        loaded (see ``offsets.last_source_build``) and the exe the
        ...
        running would pass the check ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â and the following
        ...
        try:
            ...
            from retro.engine.version_changer import fixer as _vc_fixer
            running = r.get_running_build_string()
            offsets_build = offsets.last_source_build()
    ...
    def _revert_flag_in_memory(self, flag):
        ...
        if original is None or str(original) == '':
            from retro.utils.helpers import get_default_value
    ...
    def disable_all_live(self):
        ...
        Returns a summary dict {total, reverted}. Skips JSON sync ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the caller
    ...
    def revert_flags_in_memory(self, flags):
        ...
        'enabled', or save ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the caller is about to replace user_flags with the
    ...
    def revert_one_to_original(self, name):
        ...
        if flag_type in ('string', 'unknown'):
            # No in-memory representation we can safely write ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â JSON-only flags
        ...
        # The flag stays enabled ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the watchdog now enforces the original value
    ...
    def save_user_flags(self, skip_sync=False):
        try:
            ...
            try:
                from retro.interface.bridge import _r1_filter
    ...
    def sync_json_to_roblox(self, roblox_manager=None):
        ...
        try:
            ...
            return roblox.apply_fflags_json(flags_dict)
        except Exception as e:
    ...
    def load_offsets(self, force_cdn=False):
        ...
        try:
            from retro.utils.helpers import get_flag_prefix
            from retro.engine import offsets

            log("[*] Loading flag definitions (Imtheo)...", (100, 255, 255))
            known = offsets.load_known_flag_names()
            ...
            if self.preset_flags_list:
                ...
                log("[!] No flag list from Imtheo or cache ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â UI search limited", (255, 200, 100))

            # Re-sync existing user flags' types and clear stale unavailable markers.
            # Only adopt the official type if it's a real type ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â never let an 'unknown'
            ...
            from retro.utils.helpers import infer_type_from_name, infer_type
            with self._lock:
                for f in self.user_flags:
                    f['_status'] = None
                    ...
                    if official and official != 'unknown':
                        ...
                        # Prefix-less dump ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ heal a stored 'unknown' type from the
    ...
    def start_watchdog(self, roblox_manager):
        ...
        log("[*] Watchdog daemon started ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â enforcing DF flags.", (100, 255, 255))
    ...
    def _watchdog_loop(self):
        ...
        while self._watchdog_running:
            ...
            # memory ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â never let the watchdog write concurrently with an apply.
            ...
            for flag in enforce_list:
                ...
                # The JSON path already covers these at launch ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â re-trying every 5s is wasted work.
                ...
                # addr_data is a list (legacy multi-address shape) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â now always
                ...
                for addr_entry in addr_data:
                    ...
                    # Turbo: read-before-write ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â skip flags already correct so the
    ...
    def _hotkey_loop(self):
        ...
        VK_MAP = {
            ...
            'KeyÃƒÆ’Ã¢â‚¬Å¾Ãƒâ€¦Ã‚Â¾': 0xDB, 'KeyÃƒÆ’Ã†â€™Ãƒâ€¦Ã¢â‚¬Å“': 0xDD, 'KeyÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€¦Ã‚Â¾': 0xBA, 'KeyÃƒÆ’Ã¢â‚¬Å¾Ãƒâ€šÃ‚Â°': 0xDE, 'KeyÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“': 0xBC, 'KeyÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡': 0xBE,
        ...
        while self._hotkey_running:
            ...
            # 2b. Kill switch ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â handled before the attachment gate so it works
            ...
            if _gated and curr_time - last_gated_log > 3.0:
                log(f"[hotkey-guard] SKIP live writes ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â {_gated_reason}",
                    (255, 200, 100))
    ...
    def apply_flags_hybrid(self, roblox_manager, skip_json=False):
        # (v4.0.5: frontend visibility heartbeat + Apply refusal removed ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â it
        ...
        try:
            ...
            if not skip_json:
                ...
                # 260 applied): FPS flags are intentionally not written ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the
                # file-based FramerateCap unlock handles FPS instead.
                if fps_skipped:
                    log(f"[ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â·] {fps_skipped} FPS flag(s) skipped ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â handled by the "
                        f"FPS unlocker (count: {len(flags_dict)})", (150, 150, 150))

                json_ok, json_msg = roblox.apply_fflags_json(flags_dict)
            ...
            # === Version-mismatch guard: skip live memory when offsets ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°Ãƒâ€šÃ‚Â  Roblox ===
            # The loaded offsets are dumped against SOME Roblox build (recorded
            # in offsets._last_source_build). If the ATTACHED Roblox PID
            # is running a DIFFERENT build, the RVAs from those offsets point
            # at wrong addresses ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â WriteProcessMemory hits random pages and
            ...
            if gated:
                log(f"[!] Live memory skipped ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â {reason}. JSON applied; live "
            ...
            if not roblox.is_attached:
                log("[*] Roblox not running ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â JSON applied, will take effect on next launch.", (255, 255, 100))
                self.flags_applied = True
                self.last_apply_time = time.time()
                return

            if not roblox.open_process_for_write():
                log("[-] Could not open Roblox for memory writes. JSON was applied.", (255, 200, 100))
                self.flags_applied = True
                self.last_apply_time = time.time()
                return
                
            base = roblox.get_roblox_base()
            if not base:
            ...
            from retro.utils.helpers import infer_type_from_name, clean_flag_name, infer_type
            target_names = []
            ...
            live_addrs = roblox.scan_live_flags(target_names, force_rescan=False)
            if live_addrs:
                missing_targets = {clean_flag_name(n) for n in target_names} - set(live_addrs.keys())
                if missing_targets:
                    live_addrs = roblox.scan_live_flags(target_names, force_rescan=True)
            ...
            for flag in flags_snapshot:
                ...
                # Skip unknown type flags ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â type could not be determined
                if flag_type == 'unknown':
                    mem_skip += 1
                    flag['_status'] = 'json_only' if flag.get('_status') == 'success' else 'unavailable'
                    continue
                    
                # (FPS flags are skipped at the top of this loop ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the file-based
                ...
                if not addr_data:
                    mem_skip += 1
                    flag['_status'] = 'json_only' if flag.get('_status') == 'success' else 'unavailable'
                    log(f"[ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â·] SKIP: {name} (type={flag_type}) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â no live address found", (180, 180, 180))
                    continue
                ...
                for addr_entry in addr_data:
                    ...
                    # what we're about to write ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â otherwise an already-applied flag
                    # would record its modified value as the "original".
                    if is_enabled and not flag.get('original_captured'):
                        orig_val = roblox.read_flag_at_address(curr_live_type, curr_abs_addr)
                    ...
                    if is_enabled:
                        ...
                        res, msg = roblox.write_flag_at_address(curr_live_type, curr_abs_addr, v_write)
                ...
                if final_res:
                    ...
                    if any(isinstance(r[1], str) and "JSON_ONLY" in r[1] for r in write_results):
                        ...
                        log(f"[ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â·] JSON-ONLY: {name} ({flag_type}) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â {detail}", (180, 180, 255))
                    else:
                        mem_fail += 1
                        flag['_status'] = 'failed'
                        log(f"[-] MEM FAIL: {name} ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â {success_msg}", (255, 100, 100))
            ...
            if total_list_count > enabled_count:
                log(f"[ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â·] Information: {total_list_count - enabled_count} flags in your list are currently DISABLED and were ignored.", (150, 150, 150))
            ...
            # flag type ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â locking a static flag is redundant but harmless (engine
            ...
            if live_addrs:
                ...
                # NÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦" count used to print a bigger N than the closing "Lock: X/M
                # coveredÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦" line ever could. Dedupe here so both numbers agree.
                ...
                if lock_targets_set:
                    ...
                    threading.Thread(
                        target=roblox.lock_dynamic_flags,
                        args=(lock_targets,),
                        daemon=True
                    ).start()
            ...
            # Apply finished (or failed) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â let the watchdog resume enforcing.
            self._applying = False
    ...
    def launch_and_apply(self, roblox_manager):
        ...
        Zero-flag path is intentional: the button is 'Launch Roblox' ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â a fresh
        ...
        if has_flags:
            ...
            log("[*] Launching Roblox (no flags configured ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â nothing to inject)...", (100, 255, 255))

        success, pid, _, _ = roblox.launch_and_patch_roblox(self.user_flags)
            ...
            if has_flags:
                ...
                while time.time() - t0 < LAUNCH_INIT_TIMEOUT_SEC:
                    ...
                    if roblox.is_roblox_running():
                        ...
                        break
                    roblox.attach()
                    if roblox.is_attached and roblox.get_roblox_base():
                        if roblox.read_memory_external(roblox.get_roblox_base(), 100):
                            initialized = True
                            break
                ...
                if initialized:
                ...
                elif not roblox.is_roblox_running():
                    # CreateProcess succeeded but Roblox closed right after launch.
                    log("[-] Roblox did not start ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â it closed right after launch.", (255, 100, 100))
                    ...
                    log("[-] Roblox is running, but Retro Injector couldn't read its memory to apply live flags.", (255, 100, 100))
                    log("    Try running Retro Injector as administrator. Your flags are in JSON and will apply on the next clean launch.", (255, 180, 100))
                    for flag in self.user_flags:
                ...
                # spawn ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the watchdog below still starts so flags added mid-game
                # apply live.
                log(f"[+] Roblox launched (PID {pid})", (100, 255, 100))
        else:
            log("[-] Launch failed ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Roblox could not be started.", (255, 200, 100))
            if has_flags:
