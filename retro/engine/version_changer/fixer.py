...
def _get_manifest_packages(target_guid: str) -> Optional[list]:
    ...
    from retro.engine.version_changer import manifest, downloader
...
def disk_space_precheck(packages, versions_root, cache_dirs):
    ...
    """
    from retro.engine.version_changer import downloader
    try:
...
def run_upgrade(target_guid: str, versions_root: str, cache_dirs: list,
    ...
    """
    from retro.engine.version_changer import downloader, installer
    from retro.utils.logger import log
    ...
    staging = tempfile.mkdtemp(prefix="Retro Injector_rbx_")
    ...
    try:
        ...
        try:
            ...
            # Another process (or an earlier Retro Injector run) landed the same build
