"""Bootstrapper mode: register Retro Injector as the Roblox web-join handler so website
joins route through Retro Injector (update-if-needed -> launch pinned build -> inject).
...
def should_seize(handler_class: str, fixable: bool) -> bool:
    """Whether to register Retro Injector as the handler now.
    - already ours -> no.
...
def repair_scheme() -> bool:
    ...
    Retro Injector) is launchable from the website.
...
def register(handler_exe: str, script: Optional[str] = None) -> dict:
    """Back up the existing commands, then register Retro Injector as the handler for every
    ...
    log("[+] Retro Injector registered as roblox / roblox-player handler", (100, 255, 100))
    return backups
...
def restore(backup) -> None:
    ...
    if restored:
        ...
        log("[+] Removed Retro Injector Roblox handler(s)", (100, 255, 100))
