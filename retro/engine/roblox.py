...
# ================================================================
# ctypes function prototypes ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â MUST be defined before first call
...
# Memory operations ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â critical for 64-bit correctness
...
# Allocate memory inside the target process ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â used to back FString flags whose
...
# Full process image name ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â used by get_running_build_string() to identify
...
# NtQueryInformationProcess ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â get PEB address for base resolution
...
# time ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â torn writes + wrong-address writes that corrupt Roblox memory and freeze
...
class RobloxManager:
    ...
    def _pick_newest_player_dir(dirs):
        ...
        mtime ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â writing ClientAppSettings.json would make leftovers win."""
        if not dirs:
            return None
        return max(
            dirs,
    ...
    def get_install_versions_root():
        """Where Retro Injector should download a Roblox build.
    ...
    def get_stock_versions_root():
        ...
        None. This is the ONLY install Retro Injector writes flag files into ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â third-party
    ...
    def get_writable_version_dirs():
        """Version dirs Retro Injector is allowed to WRITE flag files into: STOCK Roblox only.
        ...
        their version dirs at launch. Retro Injector must never overwrite those files ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â doing
        ...
        Retro Injector applies flags via live memory injection instead (never touching its
    ...
    def get_running_build_string(self):
        ...
        Roblox process ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the exe Retro Injector currently holds a handle to.
        ...
        API failed). Callers must treat 'unknown' as "no signal" ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â do NOT
    ...
    def _write_flags_to_dirs(flags_dict, vdirs, merge=False):
        ...
        file already exists there, read it, overlay `flags_dict` on top (Retro Injector
        wins conflicts), write back ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â preserving whatever else lived there
            ...
            try:
                ...
                if merge and os.path.isfile(settings_file):
                    try:
                        ...
                        # Unreadable existing file ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ just overwrite with ours
    ...
    def apply_fflags_json(flags_dict, prefer_guid=None):
        ...
        offsets don't match (see flags.apply_flags_hybrid guard).
        ...
        so Retro Injector's flags need re-applying after each update. Same lifecycle
        constraint as stock installs ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â no worse.
        """
        ...
        # No stock install ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â try the bootstrapper install Retro Injector detected.
        ...
        if success > 0:
            return True, (
                ...
                f"({os.path.basename(target[0])}) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â merged with existing settings"
            )
    ...
    def _clientapp_targets(include_missing=True):
        """All ClientAppSettings.json paths Retro Injector manages for clearing: every
        ...
        (we never CREATE the global file ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â only clear one that's already there).
        """
    ...
    def clientapp_json_has_flags():
        ...
        for p in RobloxManager._clientapp_targets(include_missing=False):
            try:
                ...
                # Unreadable / not JSON: can't prove flags ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â skip it.
                continue
        return False
    ...
    def clear_fflags_json():
        ...
        Used when Retro Injector is not actively applying flags (app exit, Roblox exit,
        ...
        # Also clear the legacy global file, but only if it already exists ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â we
    ...
    def __init__(self, pid=None):
        ...
        # arena, so this is now load-bearing ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â auto-init and let it stay None
        # only if stub construction fails on this host.
        try:
            from retro.engine.syscalls import SyscallManager
            self.syscall_manager = SyscallManager()
        except Exception as e:
            log(f"[!] SyscallManager init failed: {e} ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â falling back to standard NtWrite", (255, 200, 100))
    ...
    def is_roblox_running():
        """True if a RobloxPlayerBeta.exe process is currently alive ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â regardless
    ...
    def _launch_error_reason(err):
        ...
        return {
            ...
            5: "access denied ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â try running Retro Injector as administrator",
    ...
    def find_pattern(self, pattern_str, scan_size=None):
        ...
        skipped an ENTIRE chunk whenever any page in it was unreadable ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â and the
    ...
    def write_memory_external(self, addr, data):
        ...
        concurrently on the same 4 KB region and produce a torn write ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â
    ...
    def get_roblox_base(self):
        ...
        if not self._h_process:
            if not self.open_process_for_write():
                # PEB read needs a handle but Toolhelp32 doesn't ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â keep going.
                pass
    ...
    def scan_live_flags(self, target_names: list[str] | None = None, force_rescan: bool = False) -> dict[str, list[dict]]:
        """Locking wrapper ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â serializes the cache fast-path/rebuild against
    ...
    def _fetch_offset_sources(self, clean_targets=None):
        """Resolve flag RVAs + FFlagList struct offsets via offsets.
    ...
    def write_flag_at_address(self, flag_type, abs_addr, value):
        ...
        can downgrade the log level ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the JSON path already covers those flags.
        """
        if not self._h_process:
        ...
        if flag_type == "bool":
            ...
            try:
                ...
                # corrupting it. Engine reads the corruption on game join ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ silent exit.
    ...
    def _write_raw(self, abs_addr, data):
        """Locking wrapper ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â every process write is serialized so two threads
    ...
    def _write_raw_impl(self, abs_addr, data):
        ...
        # All paths failed ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â classify the region so the caller can decide what to do.
        ...
        IMAGE = 0x1000000
        MAPPED = 0x40000          # MEM_MAPPED ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â file-backed section (Hyperion uses this for protected flag storage)
        COMMIT = 0x1000
    ...
    def _write_std_string(self, abs_addr, value):
        ...
        intentionally leaked ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â these flags are set rarely and Roblox is about to
        ...
        log(f"[str-write] REFUSED long string ({n}B > {SSO_CAP}B) at 0x{abs_addr:X} ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â JSON path only",
            (255, 200, 100))
        ...
        #      reassigns this std::string ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â a DFString refreshed from the server,
        #      or teardown on exit ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â MSVC calls operator delete on _Ptr. Our
    ...
    def launch_and_patch_roblox(self, flags_list):
        ...
        latest = None
        try:
            from retro.engine.version_changer import deployment, fixer
        ...
        from retro.engine.version_changer.channel import pin_production_channel
        ...
        # home screen ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the same flag the official Roblox shortcut uses.
    ...
    def launch_specific_version(self, guid, args=None):
        ...
        if not exe_path:
            ...
            return False, 0
        from retro.engine.version_changer.channel import (
            pin_production_channel,
    ...
    def lock_dynamic_flags(self, target_addrs: list) -> int:
        ...
        Design (safe + fast):
          ...
          - REGION SIZE CAP (256 MB). Skip GC arenas ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â they hold random object
            data that yields few real hits per byte scanned, and eat scan time.
          - PER-REGION CHUNK CAP (32 chunks ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â 2 MB = 64 MB). Guarantees we don't
        ...
        if not target_addrs:
            return 0
        if not self._h_process:
            # Entry guard: the previous "Scheduling lock for NÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦" line promised
            # a result. If we can't run the scan at all (no process handle),
            # close the loop with an explicit line instead of silence ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â matches
            # the never-silent contract of the summary block below.
            log(f"[!] Lock: 0/{len(target_addrs)} ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â not attached to Roblox, skipped",
                (200, 180, 100))
            return 0
        ...
        # ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â either newly-patched or found already at 0x01. A Flags-off/on cycle
        ...
        if target_set <= self._locked_value_ptrs:
            log(f"[+] Lock: {len(target_set)}/{len(target_set)} already locked "
                f"from a prior scan ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â skipped",
                (100, 200, 255))
            return 0
            ...
            while (offset < region_size
                ...
                for i, val in enumerate(qwords):
                    if val in target_set:
                        ...
                        # ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â no second NtRead syscall. Saves ~300 syscalls per
                        ...
                        if current_attr == 0x01:
                            # Already locked from a previous scan ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â record so
        ...
        # Always emit a closing line so the earlier "Scheduling lock for NÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦"
        ...
        if aborted_reason == 'timeout' and missing > 0:
            ...
            tag = " (no matching pointers ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â flags not yet in heap)"
        else:
            tag = ""
        ...
        # if Roblox reset it. No logging ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â silent by user request.
    ...
    def _verify_locks_loop(self):
        ...
        Silent unless Roblox actually clobbered something ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â then one line per
