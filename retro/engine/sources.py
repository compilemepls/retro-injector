...
The fetch chain is orchestrated in offsets.py; this module deliberately
...
from retro.utils.paths import resource_path
...
# URLs ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â primary sources (different hosts, all serve FFlag offsets)
...
# Our own GitHub mirror ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â last resort before disk/bundled
GITHUB_MIRROR_FFLAGS_HPP = (
    "https://raw.githubusercontent.com/compilemepls/retro-injector/main/data/FFlags.hpp"
)
...
# Our GitHub-hosted mirror (retro-injector). These URLs 404 while the
# repo lacks the endpoints folder; the loader chain treats a 404 the same
# as any other failure and falls through, so keeping them here is forward-safe.
MIRROR_BASE = "https://raw.githubusercontent.com/compilemepls/retro-injector/main/endpoints"
...
# the loader. Tiered per the Retro Injector 4-source policy:
...
#   Tail:   legacy Retro Injector data/FFlags.hpp GitHub mirror (kept for existing-user builds)
...
PRIMARY_NETWORK_SOURCES = [
    # Tier 1 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â imtheo direct
    (SRC_IMTHEO_DEV_REQUESTS,           SRC_IMTHEO_DEV_CURL,           IMTHEO_DEV_FFLAGS_HPP),
    (SRC_IMTHEO_REQUESTS,               SRC_IMTHEO_CURL,               IMTHEO_FFLAGS_HPP),
    # Tier 2 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â imtheo GitHub mirror we made
    (SRC_MIRROR_IMTHEO_REQUESTS,        SRC_MIRROR_IMTHEO_CURL,        MIRROR_IMTHEO_FFLAGS_HPP),
    # Tier 3 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â other direct dumpers
    ...
    # Tier 4 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â other GitHub mirrors we made
    ...
    # Tail ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â legacy Retro Injector-internal mirror (existing shipped chain, do not remove)
    (SRC_GITHUB_REQUESTS,               SRC_GITHUB_CURL,               GITHUB_MIRROR_FFLAGS_HPP),
]

# Bundled baseline (shipped inside _MEIPASS via PyInstaller --add-data=retro/data)
BUNDLED_BASELINE_PATH = resource_path(os.path.join("retro", "data", "FFlags_baseline.hpp"))

# 5 MB cap shared across sources (defensive ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â body is regex-scanned downstream)
...
_MIRROR_REPO_PATH_PREFIX = "/compilemepls/retro-injector/main/endpoints/"
...
def _mirror_meta_verified(url: str) -> bool:
    ...
    mirror body is never consumed. Honors the RETRO_TRUST_MIRROR=1 env var as a
    one-session override.
    """
    if _os.environ.get("RETRO_TRUST_MIRROR") == "1":
        return True
    try:
        p = urlparse(url)
...
def fetch_via_requests(url: str) -> Optional[bytes]:
    ...
    try:
        resp = requests.get(
            ...
            stream=True,
            headers={"User-Agent": "Retro-Offset-Loader/1.0"},
        )
...
def fetch_via_curl(url: str) -> Optional[bytes]:
    ...
    try:
        result = subprocess.run(
            [curl_path, "-fsSL", "--max-time", str(CURL_TIMEOUT), "-A", "Retro-Offset-Loader/1.0", url],
            capture_output=True,
